<template>
  <div class="row bg-background rounded-3 py-3">
    <h2 class="col-12 pt-3 text-primary fw-bolder">優惠券管理</h2>
    <div class="col-12">
      ...
      <div class="row">
        <div class="list-item-container col-5">
          <ul class="list-group" v-for="coupon in coupons" :key="coupon.id + 1">
            <li class="list-group-item me-2 text-secondary" @click="openCoupon(coupon)">
              <span class="badge bg-success me-2">{{coupon.is_enabled == 1 ? '開啟' : '關閉'}}</span>
              {{coupon.title}}
              <span class="arrow text-success">&rsaquo;</span>
            </li>
          </ul>
          <!-- 分頁元件 -->
